...
void print_preamblek (int rank){
    ...
    if (options.show_size) {
        fprintf(stdout, "%-*s", 10, "# Size");
        fprintf(stdout, "%*s", FIELD_WIDTH, "Avg Latency(us)");
        fprintf(stdout, "%*s", FIELD_WIDTH, "Avg 1000");
        fprintf(stdout, "%*s\n", FIELD_WIDTH, "sigma");
    }
}
...
void print_statsk (int rank, int size, double avg_time, double avgk, double sigma)
{
    if (rank) {
        return;
    }
    ...
    if(options.show_size){
        fprintf(stdout, "%-*d", 10, size);
        fprintf(stdout, "%*.*f%*.*f%*.*f\n", FIELD_WIDTH, FLOAT_PRECISION, avg_time, FIELD_WIDTH, FLOAT_PRECISION, avgk, FIELD_WIDTH, FLOAT_PRECISION, sigma);
    }
